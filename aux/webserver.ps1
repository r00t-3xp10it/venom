...
$Banner = @"
 ...
 ===  ===  === ======   =======   =====  ======   =======  ===  === ======   ======= 
  ===========  ===      ===  ===     === ===      === ===   ======  ===      === === 
   ==== ====   ======== =======  ======  ======== ===  ===    ==    ======== ===  === $CmdletVersion
          Simple (SE) HTTP WebServer by:r00t-3xp10it {SSA RedTeam@2020}
...
If(-not($Installation) -or $Installation -ieq $null){
   ...
   If($SEnum -ieq "True"){
      ...
      If($LASTEXITCODE -eq 0){
         ...
         $FirewallProfile = netsh advfirewall firewall show rule name="python.exe"|findstr "Profiles:"
         $FirewallState = netsh advfirewall firewall show rule name="python.exe"|findstr "Enabled:"
         $FirewallDirection = netsh advfirewall firewall show rule name="python.exe"|findstr "Direction:"
         $FirewallProto = netsh advfirewall firewall show rule name="python.exe"|findstr "Protocol:"
         $FirewallAction = netsh advfirewall firewall show rule name="python.exe"|findstr "Action:"
         $dataone = $FirewallRule -replace '                           ',''
         $datatwo = $FirewallState -replace ':                              ','  : '
         $datatre = $FirewallDirection -replace '                           ',''
